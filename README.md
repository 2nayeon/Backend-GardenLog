...
## 3. 주요 기능 및 엔터티 구조
- **내 주변 텃밭 찾기:** 네이버 지역 검색 API를 활용하여 현재 위치 기준 인근의 주말농장 및 텃밭 위치 조회 기능 제공.
- **텃밭 및 작물 관리:** 각 작물별 '개별 물주기' 및 '전체 물주기' 기능을 통해 물 준 횟수와 최종 급수일을 실시간 데이터베이스에 반영.
- **작물 백과사전:** 사용자가 작물의 카테고리(채소/과일/허브) 및 재배 난이도별로 손쉽게 필터링하여 생육 정보를 조회할 수 있는 백과사전 기능.
- **농사 일지 및 캘린더:** 월별 활동 분석 통계(물주기, 비료주기 등) 인사이트와 일지 작성 누적 수에 따른 유저 등급 자동 빌드 시스템 반영.
<br>
<br>
